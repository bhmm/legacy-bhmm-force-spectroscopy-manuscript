function log_posterior = compute_log_posterior(model, data, options, use_state_assignments)
% Compute the log posterior of the HMM given the data.
%
% log_posterior = compute_log_posterior(model, data, options, use_state_assignments)
%
% ARGUMENTS
...
%
% RETURNS
%   log_posterior (double) - log-likelihood of the HMM 'model' given the data 'data', or P(O | \Theta)
%
% NOTE
...
% Compute transition matrix prior.
% TODO: Check detailed balance.

% Compute emission prior.
for i = 1:model.nstates
  log_posterior = log_posterior - log(model.states{i}.sigma);
end
...
for trajectory_index = 1:length(data)
  ...
  if use_state_assignments
    ...
    log_posterior = log_posterior + bhmm_helper.computeTrajectoryLogLikelihood(s_t, o_t, mu, sigma, model.logPi, model.logTij, options.equilibrium);
  else
    log_posterior = log_posterior + bhmm_helper.computeModelLogLikelihood(o_t, mu, sigma, model.logPi, model.logTij, options.equilibrium);
  end
end
